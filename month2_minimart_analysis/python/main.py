...
def simulate_orders(connection):
    ...
    # Make sure product_id values exist in your products table
    sample_orders = [
        {"customer_id": 1, "product_id": 1, "product": "Milk", "price": 80, "quantity": 20},
        {"customer_id": 2, "product_id": 2, "product": "Laptop", "price": 650000, "quantity": 1},
        {"customer_id": 3, "product_id": 3, "product": "Notebook", "price": 1000, "quantity": 11},
    ]

    # Insert into database
    insert_simulated_orders(connection, sample_orders)

    print(" Simulated Orders Summary:")
    for order in sample_orders:
        total = calculate_order_total(order["price"], order["quantity"])
        large_flag = is_large_order(total, threshold=100000)  # Adjusted for Naira
        discounted_price = apply_discount(order["price"], order["quantity"])
        #converted_total = convert_currency(total, target_currency="NGN")  # Convert to Naira (same currency, but shows logic)
        converted_total = total  # Since prices are already in Naira

        print(f"- Customer {order['customer_id']} | {order['product']} x {order['quantity']}")
        print(f"  Total Price: ₦{total:,.2f}")
        print(f"  Discounted Unit Price (if applicable): ₦{discounted_price:,.2f}")
        print(f"  Total in Naira (converted): ₦{converted_total:,.2f}")
        print(f"  Large Order: {'✅ Yes' if large_flag else '❌ No'}\n")


def main():
    """Main workflow for the retail reporting system."""
    connection = None
    try:
        ...
        print(" Retail Data Summary Report:")
        ...
        for customer, revenue in report['revenue_per_customer'].items():
            naira_value = convert_currency(revenue, target_currency='NGN')
            print(f"  - {customer}: ₦{naira_value:,.2f}")
